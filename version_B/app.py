...
/* Wizard (Version B): stacked steps; inactive panels hidden so inputs stay registered */
.wizard-panel-hidden { display: none !important; }
.wizard-top-bar { flex-shrink: 0; background: var(--card-bg); box-shadow: var(--shadow-sm); }
.wizard-footer-bar { flex-shrink: 0; background: var(--card-bg); }
.wizard-step-badge { font-size: .82rem; color: var(--text-muted); font-weight: 500; }
"""
...
<script>
  ...
  if (!isLocal) {{
      ...
      gtag('js', new Date());
      gtag('config', 'G-M4BHM6T44E', {{ 'user_properties': {{ 'ab_group': 'task_version_b' }} }});
  }}
...
# Step content (all steps mounted; CSS hides inactive panels so Shiny inputs remain valid)
user_guide_body = ui.div(
        ui.div(
        ...
        ui.div(
            ...
            ui.tags.p("Changes are applied sequentially as you move through the steps. Use Reset Data on the Cleaning step to start over.", style="margin:0; font-size:.85rem; color:var(--text-muted)"),
...
data_upload_body = ui.layout_sidebar(
        ui.sidebar(
...
cleaning_body = ui.layout_sidebar(
        ui.sidebar(
            ui.accordion(
                ui.accordion_panel(
                    "Missing Values",
...
feature_eng_body = ui.layout_sidebar(
        ui.sidebar(
            ui.accordion(
                ui.accordion_panel(
...
app_ui = ui.page_sidebar(
    ...
    ui.div(
        ui.div(
            ui.div(
                ui.h4("Data Explorer Pro", class_="mb-0", style="font-weight:700; color:var(--text-primary);"),
                ui.output_ui("wizard_step_indicator"),
                class_="d-flex flex-column gap-1",
            ),
            ui.output_ui("dynamic_nav_button"),
            class_="wizard-top-bar px-4 py-3 d-flex flex-wrap justify-content-between align-items-center gap-2",
        ),
        ui.div(
            ui.output_ui("wizard_main"),
            class_="flex-grow-1",
            style="min-height: 50vh; overflow: auto;",
        ),
        ui.div(
            ui.output_ui("wizard_footer_bar"),
            class_="wizard-footer-bar px-4 py-3",
        ),
    ...
    title=None,
    fillable=True,
    window_title="Data Explorer Pro",
)
...
def server(input, output, session):
        ...
        return ui.modal(
            ui.markdown(
               "### Welcome to Data Explorer Pro! 🚀\n\n"
                ...
                "👉 **Getting Started:** Click **Start processing data** at the bottom, then use **Next** to move through "
                "upload, cleaning, feature engineering, EDA, and export in order. Open the **Mission Board** (sidebar) "
   ...
   #-------GA,2----------
    ...
    @output
    @render.ui
    def wizard_step_indicator():
        step = wizard_step_index()
        labels = ["Welcome", "Data upload", "Cleaning", "Feature engineering", "EDA", "Export"]
        return ui.div(
            ui.span(f"Step {step + 1} of 6 — {labels[step]}", class_="wizard-step-badge"),
        )
    ...
    def wizard_main():
        ...
        def pane_class(i):
            return "wizard-panel" if step == i else "wizard-panel wizard-panel-hidden"

        return ui.div(
            ui.div(user_guide_body, class_=pane_class(0)),
            ui.div(data_upload_body, class_=pane_class(1)),
            ui.div(cleaning_body, class_=pane_class(2)),
            ui.div(feature_eng_body, class_=pane_class(3)),
            ui.div(eda_body, class_=pane_class(4)),
            ui.div(export_body, class_=pane_class(5)),
        )

    @output
    @render.ui
    def wizard_footer_bar():
        step = wizard_step_index()
        st0 = "" if step == 0 else "display: none !important;"
        st_prev = "" if step >= 1 else "display: none !important;"
        st_next = "" if 1 <= step <= 4 else "display: none !important;"
        st_hint = "" if step == 5 else "display: none !important;"
        return ui.div(
            ui.div(
                ui.input_action_button("btn_start_processing", "Start processing data", class_="btn-primary btn-lg"),
                class_="d-flex justify-content-center flex-grow-1",
                style=st0,
            ),
            ui.div(
                ui.input_action_button("wizard_prev", "Back", class_="btn-outline-secondary"),
                style=st_prev,
            ),
            ui.div(
            ...
            ui.div(
                ui.p(
                    "Final step — use Download as CSV above when you are ready.",
                    class_="text-muted small mb-0 text-md-end",
                ),
                class_="flex-grow-1 text-md-end",
                style=st_hint,
            ),
            class_="d-flex align-items-center justify-content-between flex-wrap gap-3 w-100",
        )

    @reactive.Effect
    @reactive.event(input.btn_start_processing)
    def wizard_start():
        wizard_step_index.set(1)
